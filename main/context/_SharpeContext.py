...
class SharpeContext(Context):
    ...
    def __init__(self, portfolio: Portfolio, sigma: jnp.ndarray, r: jnp.ndarray, risk_free_rate:float, target_return:float=0.0) -> None:
    ...
    def ineq_constraints(self, x:jnp.ndarray) -> jnp.ndarray:
        ...
        return jnp.array([self.risk_free_rate+self.target_return-(x.T @ self.r), *(0.0-x)])
